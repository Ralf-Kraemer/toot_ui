...
/// Widget that displays media resources from a Toot
...
class _MediaContainerState extends State<MediaContainer>
  ...
  Widget build(BuildContext context) {
    ...
    Widget? child;
    if (widget.tootVM.getDisplayToot().hasPhoto) {
      switch (widget.tootVM.getDisplayToot().allPhotos.length) {
        case 1:
          child = AspectRatio(
            aspectRatio: MediaContainer.DEFAULT_ASPECT_RATIO_MEDIA_CONTAINER,
            child: _buildSinglePhoto(context,
                widget.tootVM.getDisplayToot().allPhotos, 0, hashcode),
          );
          break;
        case 2:
          child = AspectRatio(
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: _buildSinglePhoto(context,
                      widget.tootVM.getDisplayToot().allPhotos, 1, hashcode),
                )
              ],
            ),
          );
                    ...
                    children: <Widget>[
                      Expanded(
                        child: _buildSinglePhoto(
                            context,
                            widget.tootVM.getDisplayToot().allPhotos,
                            1,
                            hashcode),
                      ),
  ...
  /// hashcode - used for the Hero tag. The image URL is not enough - for example, you can still have duplicated toots on a list.
